//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';


/// tests for ObjectEzsigntemplateglobalApi
void main() {
  // final instance = ObjectEzsigntemplateglobalApi();

  group('tests for ObjectEzsigntemplateglobalApi', () {
    // Retrieve Ezsigntemplateglobals and IDs
    //
    // Get the list of Ezsigntemplateglobal to be used in a dropdown or autocomplete control.
    //
    //Future<EzsigntemplateglobalGetAutocompleteV2Response> ezsigntemplateglobalGetAutocompleteV2(String sSelector, { String eFilterActive, String sQuery, HeaderAcceptLanguage acceptLanguage }) async
    test('test ezsigntemplateglobalGetAutocompleteV2', () async {
      // TODO
    });

    // Retrieve an existing Ezsigntemplateglobal
    //
    // 
    //
    //Future<EzsigntemplateglobalGetObjectV2Response> ezsigntemplateglobalGetObjectV2(int pkiEzsigntemplateglobalID) async
    test('test ezsigntemplateglobalGetObjectV2', () async {
      // TODO
    });

  });
}
