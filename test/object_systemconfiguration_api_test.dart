//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';


/// tests for ObjectSystemconfigurationApi
void main() {
  // final instance = ObjectSystemconfigurationApi();

  group('tests for ObjectSystemconfigurationApi', () {
    // Edit an existing Systemconfiguration
    //
    // 
    //
    //Future<CommonResponse> systemconfigurationEditObjectV1(int pkiSystemconfigurationID, SystemconfigurationEditObjectV1Request systemconfigurationEditObjectV1Request) async
    test('test systemconfigurationEditObjectV1', () async {
      // TODO
    });

    // Retrieve an existing Systemconfiguration
    //
    // 
    //
    //Future<SystemconfigurationGetObjectV2Response> systemconfigurationGetObjectV2(int pkiSystemconfigurationID) async
    test('test systemconfigurationGetObjectV2', () async {
      // TODO
    });

  });
}
