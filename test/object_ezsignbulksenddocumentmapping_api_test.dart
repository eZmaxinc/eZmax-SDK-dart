//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';


/// tests for ObjectEzsignbulksenddocumentmappingApi
void main() {
  // final instance = ObjectEzsignbulksenddocumentmappingApi();

  group('tests for ObjectEzsignbulksenddocumentmappingApi', () {
    // Create a new Ezsignbulksenddocumentmapping
    //
    // The endpoint allows to create one or many elements at once.
    //
    //Future<EzsignbulksenddocumentmappingCreateObjectV1Response> ezsignbulksenddocumentmappingCreateObjectV1(EzsignbulksenddocumentmappingCreateObjectV1Request ezsignbulksenddocumentmappingCreateObjectV1Request) async
    test('test ezsignbulksenddocumentmappingCreateObjectV1', () async {
      // TODO
    });

    // Delete an existing Ezsignbulksenddocumentmapping
    //
    // 
    //
    //Future<CommonResponse> ezsignbulksenddocumentmappingDeleteObjectV1(int pkiEzsignbulksenddocumentmappingID) async
    test('test ezsignbulksenddocumentmappingDeleteObjectV1', () async {
      // TODO
    });

    // Retrieve an existing Ezsignbulksenddocumentmapping
    //
    // 
    //
    //Future<EzsignbulksenddocumentmappingGetObjectV2Response> ezsignbulksenddocumentmappingGetObjectV2(int pkiEzsignbulksenddocumentmappingID) async
    test('test ezsignbulksenddocumentmappingGetObjectV2', () async {
      // TODO
    });

  });
}
