//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for CreditcardclientRequestCompound
void main() {
  // final instance = CreditcardclientRequestCompound();

  group('test CreditcardclientRequestCompound', () {
    // The unique ID of the Creditcardclient
    // int pkiCreditcardclientID
    test('to test the property `pkiCreditcardclientID`', () async {
      // TODO
    });

    // The creditcard token identifier
    // String fksCreditcardtokenID
    test('to test the property `fksCreditcardtokenID`', () async {
      // TODO
    });

    // Whether if it's an relationisdefault
    // bool bCreditcardclientrelationIsdefault
    test('to test the property `bCreditcardclientrelationIsdefault`', () async {
      // TODO
    });

    // The description of the Creditcardclient
    // String sCreditcardclientDescription
    test('to test the property `sCreditcardclientDescription`', () async {
      // TODO
    });

    // Whether the creditcardclient is active or not
    // bool bCreditcardclientIsactive
    test('to test the property `bCreditcardclientIsactive`', () async {
      // TODO
    });

    // Whether if it's an allowedagencypayment
    // bool bCreditcardclientAllowedagencypayment
    test('to test the property `bCreditcardclientAllowedagencypayment`', () async {
      // TODO
    });

    // Whether if it's an allowedroyallepageprotection
    // bool bCreditcardclientAllowedroyallepageprotection
    test('to test the property `bCreditcardclientAllowedroyallepageprotection`', () async {
      // TODO
    });

    // Whether if it's an allowedtranquillit
    // bool bCreditcardclientAllowedtranquillit
    test('to test the property `bCreditcardclientAllowedtranquillit`', () async {
      // TODO
    });

    // CreditcarddetailRequest objCreditcarddetail
    test('to test the property `objCreditcarddetail`', () async {
      // TODO
    });

    // The creditcard card CVV
    // String sCreditcardclientCVV
    test('to test the property `sCreditcardclientCVV`', () async {
      // TODO
    });


  });

}
