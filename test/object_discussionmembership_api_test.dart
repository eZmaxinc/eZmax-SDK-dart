//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';


/// tests for ObjectDiscussionmembershipApi
void main() {
  // final instance = ObjectDiscussionmembershipApi();

  group('tests for ObjectDiscussionmembershipApi', () {
    // Create a new Discussionmembership
    //
    // The endpoint allows to create one or many elements at once.
    //
    //Future<DiscussionmembershipCreateObjectV1Response> discussionmembershipCreateObjectV1(DiscussionmembershipCreateObjectV1Request discussionmembershipCreateObjectV1Request) async
    test('test discussionmembershipCreateObjectV1', () async {
      // TODO
    });

    // Delete an existing Discussionmembership
    //
    // 
    //
    //Future<CommonResponse> discussionmembershipDeleteObjectV1(int pkiDiscussionmembershipID) async
    test('test discussionmembershipDeleteObjectV1', () async {
      // TODO
    });

  });
}
