//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for CommonResponseErrorWrongFranchiseoffice
void main() {
  // final instance = CommonResponseErrorWrongFranchiseoffice();

  group('test CommonResponseErrorWrongFranchiseoffice', () {
    // The message giving details about the error
    // String sErrorMessage
    test('to test the property `sErrorMessage`', () async {
      // TODO
    });

    // FieldEErrorCode eErrorCode
    test('to test the property `eErrorCode`', () async {
      // TODO
    });

    // More error message detail
    // List<String> aSErrorMessagedetail (default value: const [])
    test('to test the property `aSErrorMessagedetail`', () async {
      // TODO
    });

    // The unique ID of the Franchiseagence
    // int fkiFranchiseagenceID
    test('to test the property `fkiFranchiseagenceID`', () async {
      // TODO
    });

    // The name of the Franchiseagence
    // String sFranchiseagenceName
    test('to test the property `sFranchiseagenceName`', () async {
      // TODO
    });

    // The unique ID of the Franchisereoffice
    // int fkiFranchiseofficeID
    test('to test the property `fkiFranchiseofficeID`', () async {
      // TODO
    });

    // The code of the Franchiseoffice
    // String iFranchiseofficeCode
    test('to test the property `iFranchiseofficeCode`', () async {
      // TODO
    });


  });

}
