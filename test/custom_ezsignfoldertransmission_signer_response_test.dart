//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for CustomEzsignfoldertransmissionSignerResponse
void main() {
  // final instance = CustomEzsignfoldertransmissionSignerResponse();

  group('test CustomEzsignfoldertransmissionSignerResponse', () {
    // The unique ID of the User
    // int fkiUserID
    test('to test the property `fkiUserID`', () async {
      // TODO
    });

    // The First name of the contact
    // String sContactFirstname
    test('to test the property `sContactFirstname`', () async {
      // TODO
    });

    // The Last name of the contact
    // String sContactLastname
    test('to test the property `sContactLastname`', () async {
      // TODO
    });


  });

}
