//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for BrandingResponseCompoundV3
void main() {
  // final instance = BrandingResponseCompoundV3();

  group('test BrandingResponseCompoundV3', () {
    // The unique ID of the Branding
    // int pkiBrandingID
    test('to test the property `pkiBrandingID`', () async {
      // TODO
    });

    // The unique ID of the Email
    // int fkiEmailID
    test('to test the property `fkiEmailID`', () async {
      // TODO
    });

    // MultilingualBrandingDescription objBrandingDescription
    test('to test the property `objBrandingDescription`', () async {
      // TODO
    });

    // The Description of the Branding in the language of the requester
    // String sBrandingDescriptionX
    test('to test the property `sBrandingDescriptionX`', () async {
      // TODO
    });

    // The name of the Branding  This value will only be set if you wish to overwrite the default name. If you want to keep the default name, leave this property empty
    // String sBrandingName
    test('to test the property `sBrandingName`', () async {
      // TODO
    });

    // The email address.
    // String sEmailAddress
    test('to test the property `sEmailAddress`', () async {
      // TODO
    });

    // FieldEBrandingLogo eBrandingLogo
    test('to test the property `eBrandingLogo`', () async {
      // TODO
    });

    // FieldEBrandingAlignlogo eBrandingAlignlogo
    test('to test the property `eBrandingAlignlogo`', () async {
      // TODO
    });

    // The primary color. This is a RGB color converted into integer
    // int iBrandingColor
    test('to test the property `iBrandingColor`', () async {
      // TODO
    });

    // Whether the Branding is active or not
    // bool bBrandingIsactive
    test('to test the property `bBrandingIsactive`', () async {
      // TODO
    });

    // The url of the picture used as logo in the Branding
    // String sBrandingLogourl
    test('to test the property `sBrandingLogourl`', () async {
      // TODO
    });

    // The url of the picture used in email as logo in the Branding
    // String sBrandingLogoemailurl
    test('to test the property `sBrandingLogoemailurl`', () async {
      // TODO
    });

    // The url of the picture used as logo in the Branding
    // String sBrandingLogointerfaceurl
    test('to test the property `sBrandingLogointerfaceurl`', () async {
      // TODO
    });


  });

}
