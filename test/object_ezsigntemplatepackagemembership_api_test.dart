//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';


/// tests for ObjectEzsigntemplatepackagemembershipApi
void main() {
  // final instance = ObjectEzsigntemplatepackagemembershipApi();

  group('tests for ObjectEzsigntemplatepackagemembershipApi', () {
    // Create a new Ezsigntemplatepackagemembership
    //
    // The endpoint allows to create one or many elements at once.
    //
    //Future<EzsigntemplatepackagemembershipCreateObjectV1Response> ezsigntemplatepackagemembershipCreateObjectV1(EzsigntemplatepackagemembershipCreateObjectV1Request ezsigntemplatepackagemembershipCreateObjectV1Request) async
    test('test ezsigntemplatepackagemembershipCreateObjectV1', () async {
      // TODO
    });

    // Delete an existing Ezsigntemplatepackagemembership
    //
    // 
    //
    //Future<EzsigntemplatepackagemembershipDeleteObjectV1Response> ezsigntemplatepackagemembershipDeleteObjectV1(int pkiEzsigntemplatepackagemembershipID) async
    test('test ezsigntemplatepackagemembershipDeleteObjectV1', () async {
      // TODO
    });

    // Retrieve an existing Ezsigntemplatepackagemembership
    //
    // 
    //
    //Future<EzsigntemplatepackagemembershipGetObjectV1Response> ezsigntemplatepackagemembershipGetObjectV1(int pkiEzsigntemplatepackagemembershipID) async
    test('test ezsigntemplatepackagemembershipGetObjectV1', () async {
      // TODO
    });

    // Retrieve an existing Ezsigntemplatepackagemembership
    //
    // 
    //
    //Future<EzsigntemplatepackagemembershipGetObjectV2Response> ezsigntemplatepackagemembershipGetObjectV2(int pkiEzsigntemplatepackagemembershipID) async
    test('test ezsigntemplatepackagemembershipGetObjectV2', () async {
      // TODO
    });

  });
}
