# openapi.api.ObjectAgenttypeApi
...
final api_instance = ObjectAgenttypeApi();
final sSelector = sSelector_example; // String | The type of Agenttypes to return
final eFilterActive = eFilterActive_example; // String | Specify which results we want to display.
final sQuery = sQuery_example; // String | Allow to filter the returned results
final acceptLanguage = ; // HeaderAcceptLanguage | 

try {
    final result = api_instance.agenttypeGetAutocompleteV2(sSelector, eFilterActive, sQuery, acceptLanguage);
    print(result);
} catch (e) {
    print('Exception when calling ObjectAgenttypeApi->agenttypeGetAutocompleteV2: $e\n');
}
```
