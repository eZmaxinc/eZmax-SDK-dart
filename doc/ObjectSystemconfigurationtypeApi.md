# openapi.api.ObjectSystemconfigurationtypeApi
...
final api_instance = ObjectSystemconfigurationtypeApi();
final sSelector = sSelector_example; // String | The type of Systemconfigurationtype to return
final eFilterActive = eFilterActive_example; // String | Specify which results we want to display.
final sQuery = sQuery_example; // String | Allow to filter the returned results
final acceptLanguage = ; // HeaderAcceptLanguage | 

try {
    final result = api_instance.systemconfigurationtypeGetAutocompleteV2(sSelector, eFilterActive, sQuery, acceptLanguage);
    print(result);
} catch (e) {
    print('Exception when calling ObjectSystemconfigurationtypeApi->systemconfigurationtypeGetAutocompleteV2: $e\n');
}
```
