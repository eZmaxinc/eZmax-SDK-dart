# openapi.api.ObjectPhonetypeApi
...
final api_instance = ObjectPhonetypeApi();
final sSelector = sSelector_example; // String | The type of Phonetypes to return
final eFilterActive = eFilterActive_example; // String | Specify which results we want to display.
final sQuery = sQuery_example; // String | Allow to filter the returned results
final acceptLanguage = ; // HeaderAcceptLanguage | 

try {
    final result = api_instance.phonetypeGetAutocompleteV2(sSelector, eFilterActive, sQuery, acceptLanguage);
    print(result);
} catch (e) {
    print('Exception when calling ObjectPhonetypeApi->phonetypeGetAutocompleteV2: $e\n');
}
```
