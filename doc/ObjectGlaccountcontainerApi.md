# openapi.api.ObjectGlaccountcontainerApi
...
final api_instance = ObjectGlaccountcontainerApi();
final sSelector = sSelector_example; // String | The type of Glaccountcontainers to return
final eFilterActive = eFilterActive_example; // String | Specify which results we want to display.
final sQuery = sQuery_example; // String | Allow to filter the returned results
final acceptLanguage = ; // HeaderAcceptLanguage | 

try {
    final result = api_instance.glaccountcontainerGetAutocompleteV2(sSelector, eFilterActive, sQuery, acceptLanguage);
    print(result);
} catch (e) {
    print('Exception when calling ObjectGlaccountcontainerApi->glaccountcontainerGetAutocompleteV2: $e\n');
}
```
