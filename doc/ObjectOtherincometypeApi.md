# openapi.api.ObjectOtherincometypeApi
...
final api_instance = ObjectOtherincometypeApi();
final sSelector = sSelector_example; // String | The type of Otherincometypes to return
final eFilterActive = eFilterActive_example; // String | Specify which results we want to display.
final sQuery = sQuery_example; // String | Allow to filter the returned results
final acceptLanguage = ; // HeaderAcceptLanguage | 

try {
    final result = api_instance.otherincometypeGetAutocompleteV2(sSelector, eFilterActive, sQuery, acceptLanguage);
    print(result);
} catch (e) {
    print('Exception when calling ObjectOtherincometypeApi->otherincometypeGetAutocompleteV2: $e\n');
}
```
