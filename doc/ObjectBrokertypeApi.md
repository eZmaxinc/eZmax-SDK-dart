# openapi.api.ObjectBrokertypeApi
...
final api_instance = ObjectBrokertypeApi();
final sSelector = sSelector_example; // String | The type of Brokertypes to return
final eFilterActive = eFilterActive_example; // String | Specify which results we want to display.
final sQuery = sQuery_example; // String | Allow to filter the returned results
final acceptLanguage = ; // HeaderAcceptLanguage | 

try {
    final result = api_instance.brokertypeGetAutocompleteV2(sSelector, eFilterActive, sQuery, acceptLanguage);
    print(result);
} catch (e) {
    print('Exception when calling ObjectBrokertypeApi->brokertypeGetAutocompleteV2: $e\n');
}
```
