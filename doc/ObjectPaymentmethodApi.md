# openapi.api.ObjectPaymentmethodApi
...
final api_instance = ObjectPaymentmethodApi();
final sSelector = sSelector_example; // String | The type of Paymentmethods to return
final eFilterActive = eFilterActive_example; // String | Specify which results we want to display.
final sQuery = sQuery_example; // String | Allow to filter the returned results
final acceptLanguage = ; // HeaderAcceptLanguage | 

try {
    final result = api_instance.paymentmethodGetAutocompleteV2(sSelector, eFilterActive, sQuery, acceptLanguage);
    print(result);
} catch (e) {
    print('Exception when calling ObjectPaymentmethodApi->paymentmethodGetAutocompleteV2: $e\n');
}
```
