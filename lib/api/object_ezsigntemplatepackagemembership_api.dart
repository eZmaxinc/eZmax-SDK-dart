...
class ObjectEzsigntemplatepackagemembershipApi {
  ...
  ///
  /// * [int] pkiEzsigntemplatepackagemembershipID (required):
  Future<Response> ezsigntemplatepackagemembershipGetObjectV1WithHttpInfo(int pkiEzsigntemplatepackagemembershipID,) async {
    // ignore: prefer_const_declarations
    final path = r'/1/object/ezsigntemplatepackagemembership/{pkiEzsigntemplatepackagemembershipID}'
      .replaceAll('{pkiEzsigntemplatepackagemembershipID}', pkiEzsigntemplatepackagemembershipID.toString());
  ...
  ///
  /// * [int] pkiEzsigntemplatepackagemembershipID (required):
  Future<EzsigntemplatepackagemembershipGetObjectV1Response?> ezsigntemplatepackagemembershipGetObjectV1(int pkiEzsigntemplatepackagemembershipID,) async {
    final response = await ezsigntemplatepackagemembershipGetObjectV1WithHttpInfo(pkiEzsigntemplatepackagemembershipID,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'EzsigntemplatepackagemembershipGetObjectV1Response',) as EzsigntemplatepackagemembershipGetObjectV1Response;
