...
class ObjectInscriptionnotauthenticatedApi {
  ...
  Future<Response> inscriptionnotauthenticatedGetObjectV2WithHttpInfo(int pkiInscriptionnotauthenticatedID, { Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/2/object/inscriptionnotauthenticated/{pkiInscriptionnotauthenticatedID}'
      .replaceAll('{pkiInscriptionnotauthenticatedID}', pkiInscriptionnotauthenticatedID.toString());
  ...
  /// * [int] pkiInscriptionnotauthenticatedID (required):
  ///   The unique ID of the Inscriptionnotauthenticated
  Future<InscriptionnotauthenticatedGetObjectV2Response?> inscriptionnotauthenticatedGetObjectV2(int pkiInscriptionnotauthenticatedID, { Future<void>? abortTrigger, }) async {
    final response = await inscriptionnotauthenticatedGetObjectV2WithHttpInfo(pkiInscriptionnotauthenticatedID, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'InscriptionnotauthenticatedGetObjectV2Response',) as InscriptionnotauthenticatedGetObjectV2Response;
