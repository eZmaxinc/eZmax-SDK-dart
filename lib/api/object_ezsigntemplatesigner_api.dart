...
class ObjectEzsigntemplatesignerApi {
  ...
  ///
  /// * [int] pkiEzsigntemplatesignerID (required):
  Future<Response> ezsigntemplatesignerGetObjectV1WithHttpInfo(int pkiEzsigntemplatesignerID,) async {
    // ignore: prefer_const_declarations
    final path = r'/1/object/ezsigntemplatesigner/{pkiEzsigntemplatesignerID}'
      .replaceAll('{pkiEzsigntemplatesignerID}', pkiEzsigntemplatesignerID.toString());
  ...
  Future<EzsigntemplatesignerGetObjectV1Response?> ezsigntemplatesignerGetObjectV1(int pkiEzsigntemplatesignerID,) async {
    final response = await ezsigntemplatesignerGetObjectV1WithHttpInfo(pkiEzsigntemplatesignerID,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'EzsigntemplatesignerGetObjectV1Response',) as EzsigntemplatesignerGetObjectV1Response;
