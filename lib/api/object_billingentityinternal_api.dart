...
class ObjectBillingentityinternalApi {
  ...
  Future<Response> billingentityinternalDeleteObjectV1WithHttpInfo(int pkiBillingentityinternalID,) async {
    // ignore: prefer_const_declarations
  ...
  Future<BillingentityinternalDeleteObjectV1Response?> billingentityinternalDeleteObjectV1(int pkiBillingentityinternalID,) async {
    final response = await billingentityinternalDeleteObjectV1WithHttpInfo(pkiBillingentityinternalID,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'BillingentityinternalDeleteObjectV1Response',) as BillingentityinternalDeleteObjectV1Response;
