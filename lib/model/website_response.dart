...
class WebsiteResponse {
  ...
  static WebsiteResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'pkiWebsiteID'), 'Required key "WebsiteResponse[pkiWebsiteID]" is missing from JSON.');
        assert(json[r'pkiWebsiteID'] != null, 'Required key "WebsiteResponse[pkiWebsiteID]" has a null value in JSON.');
        assert(json.containsKey(r'fkiWebsitetypeID'), 'Required key "WebsiteResponse[fkiWebsitetypeID]" is missing from JSON.');
        assert(json[r'fkiWebsitetypeID'] != null, 'Required key "WebsiteResponse[fkiWebsitetypeID]" has a null value in JSON.');
        assert(json.containsKey(r'sWebsiteAddress'), 'Required key "WebsiteResponse[sWebsiteAddress]" is missing from JSON.');
        assert(json[r'sWebsiteAddress'] != null, 'Required key "WebsiteResponse[sWebsiteAddress]" has a null value in JSON.');
        return true;
      }());
