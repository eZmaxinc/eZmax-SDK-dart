...
class EzsignfolderCreateObjectV2ResponseMPayload {
  ...
  static EzsignfolderCreateObjectV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'a_pkiEzsignfolderID'), 'Required key "EzsignfolderCreateObjectV2ResponseMPayload[a_pkiEzsignfolderID]" is missing from JSON.');
        assert(json[r'a_pkiEzsignfolderID'] != null, 'Required key "EzsignfolderCreateObjectV2ResponseMPayload[a_pkiEzsignfolderID]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignfolderCreateObjectV2ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignfolderCreateObjectV2ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
