...
class ScimServiceProviderConfigChangePassword {
  ...
  static ScimServiceProviderConfigChangePassword? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'supported'), 'Required key "ScimServiceProviderConfigChangePassword[supported]" is missing from JSON.');
        assert(json[r'supported'] != null, 'Required key "ScimServiceProviderConfigChangePassword[supported]" has a null value in JSON.');
        return true;
      }());
