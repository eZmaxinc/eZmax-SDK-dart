...
class EzsigndocumentPatchObjectV1Request {
  ...
  static EzsigndocumentPatchObjectV1Request? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objEzsigndocument'), 'Required key "EzsigndocumentPatchObjectV1Request[objEzsigndocument]" is missing from JSON.');
        assert(json[r'objEzsigndocument'] != null, 'Required key "EzsigndocumentPatchObjectV1Request[objEzsigndocument]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsigndocumentPatchObjectV1Request>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsigndocumentPatchObjectV1Request.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
