...
class EzsignfolderGetObjectV1Response {
  ...
  @override
  String toString() => 'EzsignfolderGetObjectV1Response[objDebugPayload=$objDebugPayload, objDebug=$objDebug, mPayload=$mPayload]';
  ...
  static EzsignfolderGetObjectV1Response? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objDebugPayload'), 'Required key "EzsignfolderGetObjectV1Response[objDebugPayload]" is missing from JSON.');
        assert(json[r'objDebugPayload'] != null, 'Required key "EzsignfolderGetObjectV1Response[objDebugPayload]" has a null value in JSON.');
        assert(json.containsKey(r'mPayload'), 'Required key "EzsignfolderGetObjectV1Response[mPayload]" is missing from JSON.');
        assert(json[r'mPayload'] != null, 'Required key "EzsignfolderGetObjectV1Response[mPayload]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignfolderGetObjectV1Response>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignfolderGetObjectV1Response.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
