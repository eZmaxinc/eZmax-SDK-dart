...
class CustomTimezoneWithCodeResponse {
  ...
  static CustomTimezoneWithCodeResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'sTimezoneName'), 'Required key "CustomTimezoneWithCodeResponse[sTimezoneName]" is missing from JSON.');
        assert(json[r'sTimezoneName'] != null, 'Required key "CustomTimezoneWithCodeResponse[sTimezoneName]" has a null value in JSON.');
        assert(json.containsKey(r'sCode'), 'Required key "CustomTimezoneWithCodeResponse[sCode]" is missing from JSON.');
        assert(json[r'sCode'] != null, 'Required key "CustomTimezoneWithCodeResponse[sCode]" has a null value in JSON.');
        return true;
      }());
