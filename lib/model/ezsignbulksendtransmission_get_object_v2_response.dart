...
class EzsignbulksendtransmissionGetObjectV2Response {
  ...
  @override
  String toString() => 'EzsignbulksendtransmissionGetObjectV2Response[objDebugPayload=$objDebugPayload, objDebug=$objDebug, mPayload=$mPayload]';
  ...
  static EzsignbulksendtransmissionGetObjectV2Response? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objDebugPayload'), 'Required key "EzsignbulksendtransmissionGetObjectV2Response[objDebugPayload]" is missing from JSON.');
        assert(json[r'objDebugPayload'] != null, 'Required key "EzsignbulksendtransmissionGetObjectV2Response[objDebugPayload]" has a null value in JSON.');
        assert(json.containsKey(r'mPayload'), 'Required key "EzsignbulksendtransmissionGetObjectV2Response[mPayload]" is missing from JSON.');
        assert(json[r'mPayload'] != null, 'Required key "EzsignbulksendtransmissionGetObjectV2Response[mPayload]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignbulksendtransmissionGetObjectV2Response>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignbulksendtransmissionGetObjectV2Response.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
