...
class EzsignbulksendGetObjectV2ResponseMPayload {
  ...
  static EzsignbulksendGetObjectV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objEzsignbulksend'), 'Required key "EzsignbulksendGetObjectV2ResponseMPayload[objEzsignbulksend]" is missing from JSON.');
        assert(json[r'objEzsignbulksend'] != null, 'Required key "EzsignbulksendGetObjectV2ResponseMPayload[objEzsignbulksend]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignbulksendGetObjectV2ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignbulksendGetObjectV2ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
