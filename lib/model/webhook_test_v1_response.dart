...
class WebhookTestV1Response {
  ...
  static WebhookTestV1Response? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objDebugPayload'), 'Required key "WebhookTestV1Response[objDebugPayload]" is missing from JSON.');
        assert(json[r'objDebugPayload'] != null, 'Required key "WebhookTestV1Response[objDebugPayload]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<WebhookTestV1Response>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = WebhookTestV1Response.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
