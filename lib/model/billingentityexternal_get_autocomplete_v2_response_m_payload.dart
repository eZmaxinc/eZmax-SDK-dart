...
class BillingentityexternalGetAutocompleteV2ResponseMPayload {
  ...
  static BillingentityexternalGetAutocompleteV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'a_objBillingentityexternal'), 'Required key "BillingentityexternalGetAutocompleteV2ResponseMPayload[a_objBillingentityexternal]" is missing from JSON.');
        assert(json[r'a_objBillingentityexternal'] != null, 'Required key "BillingentityexternalGetAutocompleteV2ResponseMPayload[a_objBillingentityexternal]" has a null value in JSON.');
        return true;
      }());

      return BillingentityexternalGetAutocompleteV2ResponseMPayload(
        aObjBillingentityexternal: BillingentityexternalAutocompleteElementResponse.listFromJson(json[r'a_objBillingentityexternal']),
      );
    }
    return null;
  }
  ...
  static Map<String, List<BillingentityexternalGetAutocompleteV2ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = BillingentityexternalGetAutocompleteV2ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
