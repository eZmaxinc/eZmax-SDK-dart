...
class ApikeyResponse {
  ...
  static ApikeyResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'pkiApikeyID'), 'Required key "ApikeyResponse[pkiApikeyID]" is missing from JSON.');
        assert(json[r'pkiApikeyID'] != null, 'Required key "ApikeyResponse[pkiApikeyID]" has a null value in JSON.');
        assert(json.containsKey(r'fkiUserID'), 'Required key "ApikeyResponse[fkiUserID]" is missing from JSON.');
        assert(json[r'fkiUserID'] != null, 'Required key "ApikeyResponse[fkiUserID]" has a null value in JSON.');
        assert(json.containsKey(r'objApikeyDescription'), 'Required key "ApikeyResponse[objApikeyDescription]" is missing from JSON.');
        assert(json[r'objApikeyDescription'] != null, 'Required key "ApikeyResponse[objApikeyDescription]" has a null value in JSON.');
        assert(json.containsKey(r'objContactName'), 'Required key "ApikeyResponse[objContactName]" is missing from JSON.');
        assert(json[r'objContactName'] != null, 'Required key "ApikeyResponse[objContactName]" has a null value in JSON.');
        assert(json.containsKey(r'bApikeyIsactive'), 'Required key "ApikeyResponse[bApikeyIsactive]" is missing from JSON.');
        assert(json[r'bApikeyIsactive'] != null, 'Required key "ApikeyResponse[bApikeyIsactive]" has a null value in JSON.');
        assert(json.containsKey(r'objAudit'), 'Required key "ApikeyResponse[objAudit]" is missing from JSON.');
        assert(json[r'objAudit'] != null, 'Required key "ApikeyResponse[objAudit]" has a null value in JSON.');
        return true;
      }());
