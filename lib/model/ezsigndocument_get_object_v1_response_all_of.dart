...
class EzsigndocumentGetObjectV1ResponseAllOf {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "EzsigndocumentGetObjectV1ResponseAllOf[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "EzsigndocumentGetObjectV1ResponseAllOf[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<EzsigndocumentGetObjectV1ResponseAllOf> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <EzsigndocumentGetObjectV1ResponseAllOf>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = EzsigndocumentGetObjectV1ResponseAllOf.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, EzsigndocumentGetObjectV1ResponseAllOf> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = EzsigndocumentGetObjectV1ResponseAllOf.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<EzsigndocumentGetObjectV1ResponseAllOf>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<EzsigndocumentGetObjectV1ResponseAllOf>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = EzsigndocumentGetObjectV1ResponseAllOf.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
