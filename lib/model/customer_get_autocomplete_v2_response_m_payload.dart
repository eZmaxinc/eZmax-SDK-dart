...
class CustomerGetAutocompleteV2ResponseMPayload {
  ...
  static CustomerGetAutocompleteV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'a_objCustomer'), 'Required key "CustomerGetAutocompleteV2ResponseMPayload[a_objCustomer]" is missing from JSON.');
        assert(json[r'a_objCustomer'] != null, 'Required key "CustomerGetAutocompleteV2ResponseMPayload[a_objCustomer]" has a null value in JSON.');
        return true;
      }());
