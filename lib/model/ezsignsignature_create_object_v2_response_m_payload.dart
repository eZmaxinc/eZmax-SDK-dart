...
class EzsignsignatureCreateObjectV2ResponseMPayload {
  ...
  static EzsignsignatureCreateObjectV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'a_pkiEzsignsignatureID'), 'Required key "EzsignsignatureCreateObjectV2ResponseMPayload[a_pkiEzsignsignatureID]" is missing from JSON.');
        assert(json[r'a_pkiEzsignsignatureID'] != null, 'Required key "EzsignsignatureCreateObjectV2ResponseMPayload[a_pkiEzsignsignatureID]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignsignatureCreateObjectV2ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignsignatureCreateObjectV2ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
