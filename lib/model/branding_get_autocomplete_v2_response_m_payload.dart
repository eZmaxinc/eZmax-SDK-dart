...
class BrandingGetAutocompleteV2ResponseMPayload {
  ...
  static BrandingGetAutocompleteV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'a_objBranding'), 'Required key "BrandingGetAutocompleteV2ResponseMPayload[a_objBranding]" is missing from JSON.');
        assert(json[r'a_objBranding'] != null, 'Required key "BrandingGetAutocompleteV2ResponseMPayload[a_objBranding]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<BrandingGetAutocompleteV2ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = BrandingGetAutocompleteV2ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
