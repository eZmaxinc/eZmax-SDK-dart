...
class VariableexpenseCreateObjectV1ResponseMPayload {
  ...
  static VariableexpenseCreateObjectV1ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'a_pkiVariableexpenseID'), 'Required key "VariableexpenseCreateObjectV1ResponseMPayload[a_pkiVariableexpenseID]" is missing from JSON.');
        assert(json[r'a_pkiVariableexpenseID'] != null, 'Required key "VariableexpenseCreateObjectV1ResponseMPayload[a_pkiVariableexpenseID]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<VariableexpenseCreateObjectV1ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = VariableexpenseCreateObjectV1ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
