...
class EzsignfoldersignerassociationRequestCompound {
  ...
  static EzsignfoldersignerassociationRequestCompound? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'fkiEzsignfolderID'), 'Required key "EzsignfoldersignerassociationRequestCompound[fkiEzsignfolderID]" is missing from JSON.');
        assert(json[r'fkiEzsignfolderID'] != null, 'Required key "EzsignfoldersignerassociationRequestCompound[fkiEzsignfolderID]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignfoldersignerassociationRequestCompound>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignfoldersignerassociationRequestCompound.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
