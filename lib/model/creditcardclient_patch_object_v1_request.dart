...
class CreditcardclientPatchObjectV1Request {
  ...
  static CreditcardclientPatchObjectV1Request? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objCreditcardclient'), 'Required key "CreditcardclientPatchObjectV1Request[objCreditcardclient]" is missing from JSON.');
        assert(json[r'objCreditcardclient'] != null, 'Required key "CreditcardclientPatchObjectV1Request[objCreditcardclient]" has a null value in JSON.');
        return true;
      }());
