...
class ApikeyCreateObjectV2ResponseMPayload {
  ...
  static ApikeyCreateObjectV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'a_objApikey'), 'Required key "ApikeyCreateObjectV2ResponseMPayload[a_objApikey]" is missing from JSON.');
        assert(json[r'a_objApikey'] != null, 'Required key "ApikeyCreateObjectV2ResponseMPayload[a_objApikey]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<ApikeyCreateObjectV2ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = ApikeyCreateObjectV2ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
