...
class WebhookGetObjectV2ResponseMPayload {
  ...
  static WebhookGetObjectV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objWebhook'), 'Required key "WebhookGetObjectV2ResponseMPayload[objWebhook]" is missing from JSON.');
        assert(json[r'objWebhook'] != null, 'Required key "WebhookGetObjectV2ResponseMPayload[objWebhook]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<WebhookGetObjectV2ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = WebhookGetObjectV2ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
