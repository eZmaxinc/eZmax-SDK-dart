...
class ScimGroup {
  ...
  static ScimGroup? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'displayName'), 'Required key "ScimGroup[displayName]" is missing from JSON.');
        assert(json[r'displayName'] != null, 'Required key "ScimGroup[displayName]" has a null value in JSON.');
        return true;
      }());
