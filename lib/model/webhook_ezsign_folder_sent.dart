...
class WebhookEzsignFolderSent {
  ...
  static WebhookEzsignFolderSent? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objWebhook'), 'Required key "WebhookEzsignFolderSent[objWebhook]" is missing from JSON.');
        assert(json[r'objWebhook'] != null, 'Required key "WebhookEzsignFolderSent[objWebhook]" has a null value in JSON.');
        assert(json.containsKey(r'a_objAttempt'), 'Required key "WebhookEzsignFolderSent[a_objAttempt]" is missing from JSON.');
        assert(json[r'a_objAttempt'] != null, 'Required key "WebhookEzsignFolderSent[a_objAttempt]" has a null value in JSON.');
        assert(json.containsKey(r'objEzsignfolder'), 'Required key "WebhookEzsignFolderSent[objEzsignfolder]" is missing from JSON.');
        assert(json[r'objEzsignfolder'] != null, 'Required key "WebhookEzsignFolderSent[objEzsignfolder]" has a null value in JSON.');
        return true;
      }());
