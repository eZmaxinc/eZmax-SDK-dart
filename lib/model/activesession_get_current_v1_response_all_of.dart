...
class ActivesessionGetCurrentV1ResponseAllOf {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ActivesessionGetCurrentV1ResponseAllOf[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ActivesessionGetCurrentV1ResponseAllOf[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ActivesessionGetCurrentV1ResponseAllOf> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ActivesessionGetCurrentV1ResponseAllOf>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ActivesessionGetCurrentV1ResponseAllOf.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ActivesessionGetCurrentV1ResponseAllOf> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = ActivesessionGetCurrentV1ResponseAllOf.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<ActivesessionGetCurrentV1ResponseAllOf>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ActivesessionGetCurrentV1ResponseAllOf>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ActivesessionGetCurrentV1ResponseAllOf.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
