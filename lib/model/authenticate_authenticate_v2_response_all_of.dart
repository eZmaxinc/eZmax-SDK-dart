...
class AuthenticateAuthenticateV2ResponseAllOf {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AuthenticateAuthenticateV2ResponseAllOf[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AuthenticateAuthenticateV2ResponseAllOf[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<AuthenticateAuthenticateV2ResponseAllOf>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AuthenticateAuthenticateV2ResponseAllOf>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AuthenticateAuthenticateV2ResponseAllOf.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AuthenticateAuthenticateV2ResponseAllOf> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = AuthenticateAuthenticateV2ResponseAllOf.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<AuthenticateAuthenticateV2ResponseAllOf>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AuthenticateAuthenticateV2ResponseAllOf>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AuthenticateAuthenticateV2ResponseAllOf.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
