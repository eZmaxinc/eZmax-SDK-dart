...
class EzsignformfieldgroupsignerRequest {
  ...
  static EzsignformfieldgroupsignerRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'fkiEzsignfoldersignerassociationID'), 'Required key "EzsignformfieldgroupsignerRequest[fkiEzsignfoldersignerassociationID]" is missing from JSON.');
        assert(json[r'fkiEzsignfoldersignerassociationID'] != null, 'Required key "EzsignformfieldgroupsignerRequest[fkiEzsignfoldersignerassociationID]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignformfieldgroupsignerRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignformfieldgroupsignerRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
