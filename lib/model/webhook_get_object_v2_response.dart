...
class WebhookGetObjectV2Response {
  ...
  @override
  String toString() => 'WebhookGetObjectV2Response[objDebugPayload=$objDebugPayload, objDebug=$objDebug, mPayload=$mPayload]';
  ...
  static WebhookGetObjectV2Response? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objDebugPayload'), 'Required key "WebhookGetObjectV2Response[objDebugPayload]" is missing from JSON.');
        assert(json[r'objDebugPayload'] != null, 'Required key "WebhookGetObjectV2Response[objDebugPayload]" has a null value in JSON.');
        assert(json.containsKey(r'mPayload'), 'Required key "WebhookGetObjectV2Response[mPayload]" is missing from JSON.');
        assert(json[r'mPayload'] != null, 'Required key "WebhookGetObjectV2Response[mPayload]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<WebhookGetObjectV2Response>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = WebhookGetObjectV2Response.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
