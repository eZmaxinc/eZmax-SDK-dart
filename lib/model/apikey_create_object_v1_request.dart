...
class ApikeyCreateObjectV1Request {
  ...
  Map<String, dynamic> toJson() {
    ...
    if (this.objApikeyCompound != null) {
      json[r'objApikeyCompound'] = this.objApikeyCompound;
    } else {
      json[r'objApikeyCompound'] = null;
    }
    return json;
  }
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ApikeyCreateObjectV1Request[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ApikeyCreateObjectV1Request[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ApikeyCreateObjectV1Request>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ApikeyCreateObjectV1Request>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ApikeyCreateObjectV1Request.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ApikeyCreateObjectV1Request> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = ApikeyCreateObjectV1Request.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<ApikeyCreateObjectV1Request>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ApikeyCreateObjectV1Request>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ApikeyCreateObjectV1Request.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
