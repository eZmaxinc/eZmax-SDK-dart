...
class EzsignuserResponseCompound {
  ...
  static EzsignuserResponseCompound? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'pkiEzsignuserID'), 'Required key "EzsignuserResponseCompound[pkiEzsignuserID]" is missing from JSON.');
        assert(json[r'pkiEzsignuserID'] != null, 'Required key "EzsignuserResponseCompound[pkiEzsignuserID]" has a null value in JSON.');
        assert(json.containsKey(r'fkiContactID'), 'Required key "EzsignuserResponseCompound[fkiContactID]" is missing from JSON.');
        assert(json[r'fkiContactID'] != null, 'Required key "EzsignuserResponseCompound[fkiContactID]" has a null value in JSON.');
        assert(json.containsKey(r'objContact'), 'Required key "EzsignuserResponseCompound[objContact]" is missing from JSON.');
        assert(json[r'objContact'] != null, 'Required key "EzsignuserResponseCompound[objContact]" has a null value in JSON.');
        assert(json.containsKey(r'objAudit'), 'Required key "EzsignuserResponseCompound[objAudit]" is missing from JSON.');
        assert(json[r'objAudit'] != null, 'Required key "EzsignuserResponseCompound[objAudit]" has a null value in JSON.');
        return true;
      }());
