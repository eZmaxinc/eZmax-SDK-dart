...
class UNUSEDEzsignfoldersignerassociationEditObjectV1Request {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UNUSEDEzsignfoldersignerassociationEditObjectV1Request[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UNUSEDEzsignfoldersignerassociationEditObjectV1Request[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<UNUSEDEzsignfoldersignerassociationEditObjectV1Request>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <UNUSEDEzsignfoldersignerassociationEditObjectV1Request>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = UNUSEDEzsignfoldersignerassociationEditObjectV1Request.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, UNUSEDEzsignfoldersignerassociationEditObjectV1Request> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = UNUSEDEzsignfoldersignerassociationEditObjectV1Request.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<UNUSEDEzsignfoldersignerassociationEditObjectV1Request>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<UNUSEDEzsignfoldersignerassociationEditObjectV1Request>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = UNUSEDEzsignfoldersignerassociationEditObjectV1Request.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
