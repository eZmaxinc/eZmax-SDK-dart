...
class EzsignbulksendsignermappingGetObjectV2ResponseMPayload {
  ...
  static EzsignbulksendsignermappingGetObjectV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objEzsignbulksendsignermapping'), 'Required key "EzsignbulksendsignermappingGetObjectV2ResponseMPayload[objEzsignbulksendsignermapping]" is missing from JSON.');
        assert(json[r'objEzsignbulksendsignermapping'] != null, 'Required key "EzsignbulksendsignermappingGetObjectV2ResponseMPayload[objEzsignbulksendsignermapping]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignbulksendsignermappingGetObjectV2ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignbulksendsignermappingGetObjectV2ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
