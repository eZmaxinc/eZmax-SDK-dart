...
class CommonGetAutocompleteV1Response {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CommonGetAutocompleteV1Response[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CommonGetAutocompleteV1Response[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CommonGetAutocompleteV1Response> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CommonGetAutocompleteV1Response>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CommonGetAutocompleteV1Response.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CommonGetAutocompleteV1Response> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = CommonGetAutocompleteV1Response.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<CommonGetAutocompleteV1Response>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CommonGetAutocompleteV1Response>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CommonGetAutocompleteV1Response.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
