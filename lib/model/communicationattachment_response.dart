...
class CommunicationattachmentResponse {
  ...
  @override
  String toString() => 'CommunicationattachmentResponse[pkiCommunicationattachmentID=$pkiCommunicationattachmentID, fkiAttachmentID=$fkiAttachmentID, fkiInvoiceID=$fkiInvoiceID, fkiSalarypreparationID=$fkiSalarypreparationID, sCommunicationattachmentName=$sCommunicationattachmentName]';
  ...
  static Map<String, List<CommunicationattachmentResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CommunicationattachmentResponse.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
