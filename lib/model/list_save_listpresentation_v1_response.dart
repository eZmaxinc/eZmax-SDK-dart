...
class ListSaveListpresentationV1Response {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ListSaveListpresentationV1Response[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ListSaveListpresentationV1Response[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ListSaveListpresentationV1Response>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ListSaveListpresentationV1Response>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ListSaveListpresentationV1Response.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ListSaveListpresentationV1Response> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = ListSaveListpresentationV1Response.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<ListSaveListpresentationV1Response>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ListSaveListpresentationV1Response>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ListSaveListpresentationV1Response.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
