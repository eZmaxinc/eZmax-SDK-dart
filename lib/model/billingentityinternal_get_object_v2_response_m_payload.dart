...
class BillingentityinternalGetObjectV2ResponseMPayload {
  ...
  static BillingentityinternalGetObjectV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objBillingentityinternal'), 'Required key "BillingentityinternalGetObjectV2ResponseMPayload[objBillingentityinternal]" is missing from JSON.');
        assert(json[r'objBillingentityinternal'] != null, 'Required key "BillingentityinternalGetObjectV2ResponseMPayload[objBillingentityinternal]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<BillingentityinternalGetObjectV2ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = BillingentityinternalGetObjectV2ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
