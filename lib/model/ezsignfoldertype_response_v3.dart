...
class EzsignfoldertypeResponseV3 {
  /// Returns a new [EzsignfoldertypeResponseV3] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is EzsignfoldertypeResponseV3 &&
  ...
  /// Returns a new [EzsignfoldertypeResponseV3] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static EzsignfoldertypeResponseV3? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "EzsignfoldertypeResponseV3[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "EzsignfoldertypeResponseV3[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<EzsignfoldertypeResponseV3> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <EzsignfoldertypeResponseV3>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = EzsignfoldertypeResponseV3.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, EzsignfoldertypeResponseV3> mapFromJson(dynamic json) {
    final map = <String, EzsignfoldertypeResponseV3>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = EzsignfoldertypeResponseV3.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of EzsignfoldertypeResponseV3-objects as value to a dart map
  static Map<String, List<EzsignfoldertypeResponseV3>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<EzsignfoldertypeResponseV3>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = EzsignfoldertypeResponseV3.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
