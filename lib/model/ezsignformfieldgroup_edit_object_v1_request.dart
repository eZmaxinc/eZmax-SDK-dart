...
class EzsignformfieldgroupEditObjectV1Request {
  ...
  static EzsignformfieldgroupEditObjectV1Request? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objEzsignformfieldgroup'), 'Required key "EzsignformfieldgroupEditObjectV1Request[objEzsignformfieldgroup]" is missing from JSON.');
        assert(json[r'objEzsignformfieldgroup'] != null, 'Required key "EzsignformfieldgroupEditObjectV1Request[objEzsignformfieldgroup]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignformfieldgroupEditObjectV1Request>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignformfieldgroupEditObjectV1Request.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
