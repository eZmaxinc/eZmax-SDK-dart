...
class EzsignfoldersignerassociationCreateObjectV2ResponseMPayload {
  ...
  static EzsignfoldersignerassociationCreateObjectV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'a_pkiEzsignfoldersignerassociationID'), 'Required key "EzsignfoldersignerassociationCreateObjectV2ResponseMPayload[a_pkiEzsignfoldersignerassociationID]" is missing from JSON.');
        assert(json[r'a_pkiEzsignfoldersignerassociationID'] != null, 'Required key "EzsignfoldersignerassociationCreateObjectV2ResponseMPayload[a_pkiEzsignfoldersignerassociationID]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignfoldersignerassociationCreateObjectV2ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignfoldersignerassociationCreateObjectV2ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
