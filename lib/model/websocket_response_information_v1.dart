...
class WebsocketResponseInformationV1 {
  ...
  static WebsocketResponseInformationV1? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'eWebsocketMessagetype'), 'Required key "WebsocketResponseInformationV1[eWebsocketMessagetype]" is missing from JSON.');
        assert(json[r'eWebsocketMessagetype'] != null, 'Required key "WebsocketResponseInformationV1[eWebsocketMessagetype]" has a null value in JSON.');
        assert(json.containsKey(r'sWebsocketChannel'), 'Required key "WebsocketResponseInformationV1[sWebsocketChannel]" is missing from JSON.');
        assert(json[r'sWebsocketChannel'] != null, 'Required key "WebsocketResponseInformationV1[sWebsocketChannel]" has a null value in JSON.');
        assert(json.containsKey(r'mPayload'), 'Required key "WebsocketResponseInformationV1[mPayload]" is missing from JSON.');
        assert(json[r'mPayload'] != null, 'Required key "WebsocketResponseInformationV1[mPayload]" has a null value in JSON.');
        return true;
      }());
...
/// The Type of message
enum WebsocketResponseInformationV1EWebsocketMessagetypeEnum {
  responseInformationV1._(r'Response-Information-V1'),
  ;

  /// Instantiate a new enum with the provided value.
  const WebsocketResponseInformationV1EWebsocketMessagetypeEnum._(this._value);
  ...
  /// Returns the instance of [WebsocketResponseInformationV1EWebsocketMessagetypeEnum] that was successfully decoded
  ...
  /// Returns a [List] containing instances of [WebsocketResponseInformationV1EWebsocketMessagetypeEnum]
...
class WebsocketResponseInformationV1EWebsocketMessagetypeEnumTypeTransformer {
  ...
  String encode(WebsocketResponseInformationV1EWebsocketMessagetypeEnum data) => data._value;
  ...
  WebsocketResponseInformationV1EWebsocketMessagetypeEnum? decode(dynamic data, {bool allowNull = true}) {
    if (data is WebsocketResponseInformationV1EWebsocketMessagetypeEnum) {
      return data;
    }
    if (data != null) {
      switch (data) {
