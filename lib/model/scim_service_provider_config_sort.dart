...
class ScimServiceProviderConfigSort {
  ...
  static ScimServiceProviderConfigSort? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'supported'), 'Required key "ScimServiceProviderConfigSort[supported]" is missing from JSON.');
        assert(json[r'supported'] != null, 'Required key "ScimServiceProviderConfigSort[supported]" has a null value in JSON.');
        return true;
      }());
