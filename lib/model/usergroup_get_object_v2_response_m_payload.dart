...
class UsergroupGetObjectV2ResponseMPayload {
  ...
  static UsergroupGetObjectV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objUsergroup'), 'Required key "UsergroupGetObjectV2ResponseMPayload[objUsergroup]" is missing from JSON.');
        assert(json[r'objUsergroup'] != null, 'Required key "UsergroupGetObjectV2ResponseMPayload[objUsergroup]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<UsergroupGetObjectV2ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = UsergroupGetObjectV2ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
