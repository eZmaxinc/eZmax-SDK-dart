...
class WebhookGetListV1ResponseMPayload {
  ...
  static WebhookGetListV1ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'iRowReturned'), 'Required key "WebhookGetListV1ResponseMPayload[iRowReturned]" is missing from JSON.');
        assert(json[r'iRowReturned'] != null, 'Required key "WebhookGetListV1ResponseMPayload[iRowReturned]" has a null value in JSON.');
        assert(json.containsKey(r'iRowFiltered'), 'Required key "WebhookGetListV1ResponseMPayload[iRowFiltered]" is missing from JSON.');
        assert(json[r'iRowFiltered'] != null, 'Required key "WebhookGetListV1ResponseMPayload[iRowFiltered]" has a null value in JSON.');
        assert(json.containsKey(r'a_objWebhook'), 'Required key "WebhookGetListV1ResponseMPayload[a_objWebhook]" is missing from JSON.');
        assert(json[r'a_objWebhook'] != null, 'Required key "WebhookGetListV1ResponseMPayload[a_objWebhook]" has a null value in JSON.');
        return true;
      }());
