...
class ScimServiceProviderConfigPatch {
  ...
  static ScimServiceProviderConfigPatch? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'supported'), 'Required key "ScimServiceProviderConfigPatch[supported]" is missing from JSON.');
        assert(json[r'supported'] != null, 'Required key "ScimServiceProviderConfigPatch[supported]" has a null value in JSON.');
        return true;
      }());
