...
class WebsocketResponseGetWebsocketIDV1MPayload {
  ...
  static WebsocketResponseGetWebsocketIDV1MPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'sWebsocketID'), 'Required key "WebsocketResponseGetWebsocketIDV1MPayload[sWebsocketID]" is missing from JSON.');
        assert(json[r'sWebsocketID'] != null, 'Required key "WebsocketResponseGetWebsocketIDV1MPayload[sWebsocketID]" has a null value in JSON.');
        return true;
      }());
