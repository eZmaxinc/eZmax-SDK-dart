...
class WebhookEzsignFolderDisposed {
  ...
  static WebhookEzsignFolderDisposed? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objWebhook'), 'Required key "WebhookEzsignFolderDisposed[objWebhook]" is missing from JSON.');
        assert(json[r'objWebhook'] != null, 'Required key "WebhookEzsignFolderDisposed[objWebhook]" has a null value in JSON.');
        assert(json.containsKey(r'a_objAttempt'), 'Required key "WebhookEzsignFolderDisposed[a_objAttempt]" is missing from JSON.');
        assert(json[r'a_objAttempt'] != null, 'Required key "WebhookEzsignFolderDisposed[a_objAttempt]" has a null value in JSON.');
        assert(json.containsKey(r'objEzsignfolder'), 'Required key "WebhookEzsignFolderDisposed[objEzsignfolder]" is missing from JSON.');
        assert(json[r'objEzsignfolder'] != null, 'Required key "WebhookEzsignFolderDisposed[objEzsignfolder]" has a null value in JSON.');
        return true;
      }());
