    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CommonResponseErrorSTemporaryFileUrlAllOf[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CommonResponseErrorSTemporaryFileUrlAllOf[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CommonResponseErrorSTemporaryFileUrlAllOf>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CommonResponseErrorSTemporaryFileUrlAllOf.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CommonResponseErrorSTemporaryFileUrlAllOf> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = CommonResponseErrorSTemporaryFileUrlAllOf.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
