...
class ScimServiceProviderConfigEtag {
  ...
  static ScimServiceProviderConfigEtag? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'supported'), 'Required key "ScimServiceProviderConfigEtag[supported]" is missing from JSON.');
        assert(json[r'supported'] != null, 'Required key "ScimServiceProviderConfigEtag[supported]" has a null value in JSON.');
        return true;
      }());
