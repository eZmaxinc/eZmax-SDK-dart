...
class EzsigntemplatedocumentPatchObjectV1Request {
  ...
  static EzsigntemplatedocumentPatchObjectV1Request? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objEzsigntemplatedocument'), 'Required key "EzsigntemplatedocumentPatchObjectV1Request[objEzsigntemplatedocument]" is missing from JSON.');
        assert(json[r'objEzsigntemplatedocument'] != null, 'Required key "EzsigntemplatedocumentPatchObjectV1Request[objEzsigntemplatedocument]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsigntemplatedocumentPatchObjectV1Request>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsigntemplatedocumentPatchObjectV1Request.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
