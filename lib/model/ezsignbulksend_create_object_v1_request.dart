...
class EzsignbulksendCreateObjectV1Request {
  ...
  static EzsignbulksendCreateObjectV1Request? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'a_objEzsignbulksend'), 'Required key "EzsignbulksendCreateObjectV1Request[a_objEzsignbulksend]" is missing from JSON.');
        assert(json[r'a_objEzsignbulksend'] != null, 'Required key "EzsignbulksendCreateObjectV1Request[a_objEzsignbulksend]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignbulksendCreateObjectV1Request>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignbulksendCreateObjectV1Request.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
