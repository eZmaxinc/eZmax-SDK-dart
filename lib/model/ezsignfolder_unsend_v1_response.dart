...
class EzsignfolderUnsendV1Response {
  ...
  static EzsignfolderUnsendV1Response? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objDebugPayload'), 'Required key "EzsignfolderUnsendV1Response[objDebugPayload]" is missing from JSON.');
        assert(json[r'objDebugPayload'] != null, 'Required key "EzsignfolderUnsendV1Response[objDebugPayload]" has a null value in JSON.');
        return true;
      }());
  ...
  static List<EzsignfolderUnsendV1Response> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <EzsignfolderUnsendV1Response>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = EzsignfolderUnsendV1Response.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, EzsignfolderUnsendV1Response> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = EzsignfolderUnsendV1Response.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<EzsignfolderUnsendV1Response>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<EzsignfolderUnsendV1Response>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = EzsignfolderUnsendV1Response.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
