...
class PhonetypeGetAutocompleteV2ResponseMPayload {
  ...
  static PhonetypeGetAutocompleteV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'a_objPhonetype'), 'Required key "PhonetypeGetAutocompleteV2ResponseMPayload[a_objPhonetype]" is missing from JSON.');
        assert(json[r'a_objPhonetype'] != null, 'Required key "PhonetypeGetAutocompleteV2ResponseMPayload[a_objPhonetype]" has a null value in JSON.');
        return true;
      }());
