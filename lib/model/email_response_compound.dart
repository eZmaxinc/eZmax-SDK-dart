...
class EmailResponseCompound {
  ...
  static EmailResponseCompound? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'pkiEmailID'), 'Required key "EmailResponseCompound[pkiEmailID]" is missing from JSON.');
        assert(json[r'pkiEmailID'] != null, 'Required key "EmailResponseCompound[pkiEmailID]" has a null value in JSON.');
        assert(json.containsKey(r'fkiEmailtypeID'), 'Required key "EmailResponseCompound[fkiEmailtypeID]" is missing from JSON.');
        assert(json[r'fkiEmailtypeID'] != null, 'Required key "EmailResponseCompound[fkiEmailtypeID]" has a null value in JSON.');
        assert(json.containsKey(r'sEmailAddress'), 'Required key "EmailResponseCompound[sEmailAddress]" is missing from JSON.');
        assert(json[r'sEmailAddress'] != null, 'Required key "EmailResponseCompound[sEmailAddress]" has a null value in JSON.');
        return true;
      }());
