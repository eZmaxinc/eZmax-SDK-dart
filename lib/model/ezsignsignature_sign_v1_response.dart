...
class EzsignsignatureSignV1Response {
  ...
  static EzsignsignatureSignV1Response? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objDebugPayload'), 'Required key "EzsignsignatureSignV1Response[objDebugPayload]" is missing from JSON.');
        assert(json[r'objDebugPayload'] != null, 'Required key "EzsignsignatureSignV1Response[objDebugPayload]" has a null value in JSON.');
        assert(json.containsKey(r'mPayload'), 'Required key "EzsignsignatureSignV1Response[mPayload]" is missing from JSON.');
        assert(json[r'mPayload'] != null, 'Required key "EzsignsignatureSignV1Response[mPayload]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignsignatureSignV1Response>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignsignatureSignV1Response.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
