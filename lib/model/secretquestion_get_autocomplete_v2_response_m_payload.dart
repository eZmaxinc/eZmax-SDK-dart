...
class SecretquestionGetAutocompleteV2ResponseMPayload {
  ...
  static SecretquestionGetAutocompleteV2ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'a_objSecretquestion'), 'Required key "SecretquestionGetAutocompleteV2ResponseMPayload[a_objSecretquestion]" is missing from JSON.');
        assert(json[r'a_objSecretquestion'] != null, 'Required key "SecretquestionGetAutocompleteV2ResponseMPayload[a_objSecretquestion]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<SecretquestionGetAutocompleteV2ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = SecretquestionGetAutocompleteV2ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
