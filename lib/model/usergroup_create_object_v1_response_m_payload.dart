...
class UsergroupCreateObjectV1ResponseMPayload {
  ...
  static UsergroupCreateObjectV1ResponseMPayload? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'a_pkiUsergroupID'), 'Required key "UsergroupCreateObjectV1ResponseMPayload[a_pkiUsergroupID]" is missing from JSON.');
        assert(json[r'a_pkiUsergroupID'] != null, 'Required key "UsergroupCreateObjectV1ResponseMPayload[a_pkiUsergroupID]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<UsergroupCreateObjectV1ResponseMPayload>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = UsergroupCreateObjectV1ResponseMPayload.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
