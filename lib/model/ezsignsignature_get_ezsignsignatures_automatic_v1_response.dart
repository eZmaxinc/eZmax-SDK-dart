...
class EzsignsignatureGetEzsignsignaturesAutomaticV1Response {
  ...
  @override
  String toString() => 'EzsignsignatureGetEzsignsignaturesAutomaticV1Response[objDebugPayload=$objDebugPayload, objDebug=$objDebug, mPayload=$mPayload]';
  ...
  static EzsignsignatureGetEzsignsignaturesAutomaticV1Response? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'objDebugPayload'), 'Required key "EzsignsignatureGetEzsignsignaturesAutomaticV1Response[objDebugPayload]" is missing from JSON.');
        assert(json[r'objDebugPayload'] != null, 'Required key "EzsignsignatureGetEzsignsignaturesAutomaticV1Response[objDebugPayload]" has a null value in JSON.');
        assert(json.containsKey(r'mPayload'), 'Required key "EzsignsignatureGetEzsignsignaturesAutomaticV1Response[mPayload]" is missing from JSON.');
        assert(json[r'mPayload'] != null, 'Required key "EzsignsignatureGetEzsignsignaturesAutomaticV1Response[mPayload]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EzsignsignatureGetEzsignsignaturesAutomaticV1Response>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EzsignsignatureGetEzsignsignaturesAutomaticV1Response.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
